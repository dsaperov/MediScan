...
async def test_predict():
    ...
    calls = await requester.query(MESSAGE_WITH_PHOTO.as_object())
    answer_message = calls.send_message.fetchone().text
    assert answer_message == "The most likely class is BCC."
...
async def test_rating():
    ...
    mock_delete_rating = AsyncMock()
    with patch("bot.delete_rating", mock_delete_rating):
        with patch("bot.check_rating", return_value=False):
            calls = await requester.query(MESSAGE.as_object(text="/clearrating"))
            answer_message = calls.send_message.fetchone().text
            assert answer_message == "No rating to clear."
        with patch("bot.check_rating", return_value=True):
            calls = await requester.query(MESSAGE.as_object(text="/clearrating"))
            answer_message = calls.send_message.fetchone().text
...
async def test_add_rating():
    ...
    mock_save_rating = AsyncMock()

    with (patch('bot.save_rating', mock_save_rating),
          patch.object(mock_bot, 'edit_message_text') as mock_edit_message_text,
          patch('bot.get_ratings', return_value=[3])):
